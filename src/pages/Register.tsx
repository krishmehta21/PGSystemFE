...
const Register: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await client.post<{ access_token: string; pg_id: string | null; role: string }>('/api/v1/auth/register', {
        email,
        password,
      });
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-main-bg px-4 py-12">
      ...
      <div className="w-full max-w-[400px] bg-white p-6 sm:p-10 rounded-[12px] border border-main-border shadow-[0_1px_3px_rgba(0,0,0,0.06),_0_1px_2px_rgba(0,0,0,0.04)] animate-fade-up">
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-main-text mb-2">Confirm Password</label>
            <input
              type="password"
        ...
        <p className="mt-8 text-center text-sm text-text-secondary">
          ...
          <Link to="/login" className="font-medium text-main-text hover:text-accent transition-colors">
            Sign in
          </Link>
        </p>
      </div>
